...
class InvoiceControllerIntegrationTest {
    ...
    void respondsForTestePfPayloadWithExpectedContract() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/orders/generate-invoice")
    ...
    void respondsForTestePjSimplesPayloadWithExpectedContract() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/orders/generate-invoice")
                ...
                // JURIDICA + SIMPLES_NACIONAL + totalItemsValue=5840 > 5000 → rate 0.19 → 730 * 0.19 = 138.7
    ...
    private static void assertNoEnglishKeysAppearInResponse(String json) {
        String[] forbiddenKeys = {
                "\"invoiceId\"", "\"totalItemsValue\"", "\"freightValue\"",
                "\"itemTaxValue\"", "\"unitPrice\"", "\"personType\"", "\"taxRegime\""
        };
        for (String key : forbiddenKeys) {
            assertFalse(json.contains(key),
                    "Response must not contain English key " + key + " — JSON contract is snake_case Portuguese.");
        }
    }
}
