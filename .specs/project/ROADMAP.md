...
**Status:** M1/M2/M3 closed (2026-05-23). M4 closed (2026-05-24, F-AUTH). M5 closed (2026-05-24, F-RATELIMIT; patch 2026-05-25, F-COMPOSE-HEALTHCHECK). M6 closed (2026-05-24, F-BULKHEAD + F-API-DOCS). M7 closed (2026-05-25, F-DEBUG-LOGS). M8 specced 2026-05-25 (F-CLOUDWATCH-METRICS — `.specs/features/cloudwatch-metrics/`; **next feature, not started**). **Next:** F-CLOUDWATCH-METRICS T1 — add `io.micrometer:micrometer-registry-cloudwatch2` dependency to `pom.xml` (verifies REQ-1; gate `./mvnw verify`). Execution deferred until the user kicks it off.
...
## M7 — Operator-facing log coverage
...
## M8 — AWS observability backend wiring (next, not started)

**Goal:** close the documentation-only gap left by F-OBSERVABILITY + F-AWS — make the four SLI source meters actually reach Amazon CloudWatch under namespace `InvoiceGenerator` when the service runs in AWS, so the F-AWS dashboard + alarm Terraform modules can render against real data without any change to instrumentation code or the local Prometheus path.
**Target:** completion of F-CLOUDWATCH-METRICS.
**Status:** **Next feature — planned, not started.** Spec + design + tasks already drafted; execution deferred until the user kicks it off.

### Features

**F-CLOUDWATCH-METRICS — Micrometer CloudWatch registry (profile `aws`)** — PLANNED (2026-05-25, spec + design + tasks drafted under `.specs/features/cloudwatch-metrics/`; not implemented)

- Adds `io.micrometer:micrometer-registry-cloudwatch2` (AWS SDK v2) as a runtime dependency. Spring Boot 3.5.14 BOM-managed where possible; pinned to the parent's `<micrometer.version>` otherwise. F-AWS already grants `cloudwatch:PutMetricData` on the ECS task role and references namespace `InvoiceGenerator` in `infra/terraform/modules/observability/`, so no IAM / Terraform changes are expected beyond a cross-check.
- Wires a `CloudWatchMeterRegistry` `@Bean` under `@Profile("aws")` only (AD-CWM-2). Local development, the default profile, and the test profile continue to publish only to Prometheus — both registries coexist under profile `aws` (AD-CWM-3).
- AWS credentials and region resolve via `DefaultCredentialsProvider` and `DefaultAwsRegionProviderChain` so the F-AWS ECS task role authenticates automatically with no explicit access keys in code or properties. Step interval 1 minute, batch size 20 (CloudWatch `PutMetricData` hard limit), namespace `InvoiceGenerator` — all configurable via `management.cloudwatch.metrics.export.*`.
- AD-020 cardinality budget preserved across both backends — `RateLimiterMeterFilter` is `MeterRegistry`-agnostic and applies to the composite, so per-IP synthetic rate-limiter instances stay out of CloudWatch the same way they stay out of Prometheus. New `CloudWatchCardinalityGuardTest` mirrors the existing `CardinalityGuardTest` against the CloudWatch registry.
- No new SLIs, no new meters, no new instrumentation — this is a wiring feature. The four F-OBSERVABILITY SLI source meters (`http.server.requests`, `invoice.dispatch`, `invoice.sideeffect.duration`, `invoice.generated`, `invoice.rejected`) reach CloudWatch under namespace `InvoiceGenerator` with the same tag set that goes to Prometheus locally.
- 6 vertical-slice tasks (T1 dependency · T2 profile-scoped bean + `application-aws.properties` · T3 credentials/region chain + IAM cross-check · T4 cardinality guard test coverage · T5 cross-doc consistency · T6 STATE + ROADMAP closure). Spec: `.specs/features/cloudwatch-metrics/spec.md` (REQ-1..REQ-14). Design + ADRs (AD-CWM-1..4): `.specs/features/cloudwatch-metrics/design.md`.
