...
**Current Work:** M7 — **F-DEBUG-LOGS COMPLETE** (2026-05-25, `./mvnw verify` green; 141 fast tests including `DebugLogsIntegrationTest` (2) + `ResilienceEventLoggerTest` (2); 23 / 23 requirements DLG-01..DLG-23 Verified; 5 atomic commits T1..T5). Structured `info`/`debug`/`warn` log coverage now spans controller, interactor, `TaxRateTable`, `LegacyFreightCalculator`, four outbound adapters, `IntegrationEventPublisher`, new `ResilienceEventLogger` (CB + bulkhead event listeners), and `RateLimitFilter` (trip line promoted to INFO). `APP_LOG_LEVEL` env var toggles `br.com.itau.invoicegenerator` to DEBUG without rebuild; `/actuator/loggers` POST also works without restart. `docs/observability.md` carries the per-logger catalog; AD-037 records the decision; AD-036 amended to exclude the four thin Thread.sleep adapter classes from the JaCoCo bundle.

**M8 (next, not started):** F-CLOUDWATCH-METRICS — Micrometer CloudWatch registry under `@Profile("aws")`. Spec + design + tasks drafted under `.specs/features/cloudwatch-metrics/` (REQ-1..14, T1..T6 vertical-slice, AD-CWM-1..4 *proposed* in design.md only; no code, no AD ratified yet). Library target: `io.micrometer:micrometer-registry-cloudwatch2`. Execution deferred until the user kicks it off.

**Prior milestones (all complete):** M1/M2 F-SAFETY-NET + F-UPGRADE + F-CLEAN; M2.5 F-DEFECTS-FUNCTIONAL + F-DEFECTS-PERFORMANCE; M3 F-RESILIENCE + F-OBSERVABILITY + F-AWS + F-DEPLOY-ACTION; M4 F-AUTH; M5 F-RATELIMIT (+ F-COMPOSE-HEALTHCHECK patch); M6 F-BULKHEAD + F-API-DOCS; M7 F-DEBUG-LOGS. 141 fast tests passing; Newman 14 requests / 27 assertions / 0 failures. Operator SSOTs: `docs/observability.md` (observability + debug logs catalog), `docs/bulkhead-strategy.md` (bulkhead calibration), `docs/aws-architecture.md` (AWS proposal).
