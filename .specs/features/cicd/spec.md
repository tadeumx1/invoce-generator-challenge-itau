# F-CICD — GitHub Actions AWS Deploy Pipeline Specification
...
F-CICD closes that gap with a single GitHub Actions workflow that a reviewer can read in
...
| Smoke test            | **POST the existing `teste-pf.json` payload**            | Reuses the local-test fixture the README already documents (`curl -X POST .../api/orders/generate-invoice -d @paylods/teste-pf.json`). One curl, one assertion (`curl -fsS`), no new test framework.                                                |
...
- [x] Smoke test uses the existing `src/main/resources/paylods/teste-pf.json` fixture
      via `curl -fsS --retry 5`.
- [x] `ROADMAP.md` adds F-CICD under M3 with status COMPLETE.
...
**Why P1:** F-AWS's reviewer-facing artifact answers "where does it run". F-CICD answers
"how does it get there". Together they close the deployment story.

**Acceptance Criteria:**

- [x] **CICD-01:** A single workflow file at `.github/workflows/deploy-aws.yml` contains
...
- [x] **CICD-02:** Workflow file opens with a header comment describing the three-job
...
- [x] **CICD-03:** Jobs are explicitly named (`build-and-test`, `terraform-apply`,
      `docker-deploy`) and chained via `needs:` so failure short-circuits.
- [x] **CICD-04:** YAML parses cleanly with a stdlib YAML loader.
...
- [x] **CICD-05:** Workflow declares `permissions: id-token: write` + `contents: read`
      at the workflow level (least privilege — only the jobs that need OIDC get the
      token).
- [x] **CICD-06:** Both AWS-touching jobs (`terraform-apply`, `docker-deploy`) call
...
- [x] **CICD-07:** Header comment documents that the IAM role's trust policy must
...
- [x] **CICD-08:** `terraform fmt -recursive -check` runs before `init`. A formatting
      drift fails the pipeline (matches the local F-AWS gate).
- [x] **CICD-09:** `terraform init` injects S3 backend bucket, DynamoDB lock table,
...
- [x] **CICD-10:** `terraform plan -out=tfplan` runs, the binary plan is uploaded as a
      `tfplan` artifact (7-day retention), and `terraform apply tfplan` runs against the
      saved plan so apply cannot diverge from plan.
- [x] **CICD-11:** Plan injects `account_id`, `region`, `environment` via `TF_VAR_*`
...
- [x] **CICD-12:** Image is tagged with the immutable `${{ github.sha }}` and pushed to
      ...
      references.
- [x] **CICD-13:** New task definition is rendered by `aws ecs describe-task-definition`
...
- [x] **CICD-14:** Deploy uses `aws-actions/amazon-ecs-deploy-task-definition@v2` with
      `wait-for-service-stability: true`, `wait-for-minutes: 10`. Pipeline fails if the
      service does not stabilise.
- [x] **CICD-15:** Buildx pushes a `linux/amd64` image (Fargate platform per F-AWS) and
...
- [x] **CICD-16:** A `Smoke test` step runs `curl -fsS --retry 5 --retry-delay 6
      ...
      `src/main/resources/paylods/teste-pf.json` fixture as the body.
- [x] **CICD-17:** `curl -f` makes the step fail on any 4xx/5xx response. Retries
...
- [x] **CICD-18:** `infra/terraform/outputs.tf` exposes `ecs_cluster_name` and
      `ecs_service_name` (passthrough of the existing `module.ecs.cluster_name` /
      `module.ecs.service_name`), with descriptions that mention the F-CICD pipeline as
      the consumer.
- [x] **CICD-19:** Both outputs are non-sensitive (cluster/service names are visible in
      the AWS console) so `terraform output -raw` works without `-no-color` /
      sensitive-handling tricks.
- [x] **CICD-20:** `terraform fmt -recursive -check + init -backend=false + validate`
...
4. ✅ F-AWS gate (`terraform fmt -recursive -check + init -backend=false + validate`)
   stays green.
5. ✅ `ROADMAP.md` lists F-CICD under M3 with status COMPLETE.
6. ✅ `STATE.md` records the F-CICD design decisions (OIDC over keys, commented triggers,
