...
./mvnw verify                                   # tests + Spotless + Checkstyle + JaCoCo threshold gate (CI gate)
...
**Enforced** by `./mvnw verify` (JaCoCo `check` execution; quick task `007-coverage-threshold-gate`, commit `70ce7ee`, 2026-05-25):
...
Falling below either threshold fails the build.

**Exclude policy:** the bundle-level rule sidesteps low-signal classes that would dilute the signal without surfacing real regressions. Excluded from JaCoCo measurement (see `pom.xml` `jacoco-maven-plugin > configuration > excludes`):

- Bootstrap: `**/InvoiceGeneratorApplication*`.
- Spring wiring / properties: `**/*Config*`, `**/*Properties*`.
- Adapter contracts (DTOs / requests / responses / exceptions): `**/*Dto*`, `**/*Request*`, `**/*Response*`, `**/*Exception*`.
- Use-case interfaces (no executable code): `**/*UseCase*`.
- Domain contracts and data carriers: `**/domain/port/**`, `**/domain/model/**`.
- Static envelopes and constants: `**/adapter/messaging/IntegrationEvent*`, `**/adapter/messaging/InvoiceTopics*`, `**/adapter/observability/InvoiceKafkaHeaders*`, `**/adapter/observability/RejectionCode*`.
- Demo-user data carriers: `**/adapter/security/login/DemoUser*`.

**Where the gate has teeth:** `application/`, `domain/service/`, controllers, filters (security, rate-limit, correlation-id), mappers, exception handlers, and the four outbound adapters with their resilience annotations. These are the files reviewers should open first in `target/site/jacoco/index.html`.

**Originally deferred plan vs. shipped:** earlier docs proposed per-layer thresholds (≥80 % domain/use-case, ≥60 % adapter). The shipped gate is uniform at the bundle level (85 % / 75 %) with the exclude list above; see AD-036 in `STATE.md` for the rationale.
...
`./mvnw verify` runs tests, Spotless check, Checkstyle, jar packaging, JaCoCo report generation, **and** the JaCoCo coverage gate (≥ 85 % line / ≥ 75 % branch at the bundle level — see [Coverage Targets](#coverage-targets)).
